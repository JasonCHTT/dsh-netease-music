...
export function apply(ctx, config = {}) {
  ...
  /** Loopback hosts the plugin answers; extend via `config.allowedHosts`. */
  const LOOPBACK_HOSTS = new Set(["localhost", "127.0.0.1", "::1", "[::1]"]);

  /**
   * Settings minus the login cookie. The browser half never needs the cookie
   * literal — the host attaches it to every proxied API call — so returning it
   * from same-origin routes would only widen the attack surface (any
   * same-origin script, XSS or browser extension could steal the Netease
   * login). All routes that hand `settings` to the browser go through this.
   */
  function safeSettings() {
    const { cookie: _cookie, ...safe } = settings;
    return safe;
  }

  /**
   * Host-header gate: only loopback (or hosts explicitly listed in
   * `config.allowedHosts`) may call the plugin routes. Blocks DNS rebinding and
   * proxy forwarding, where the browser or a middlebox addresses the local
   * server under a foreign Host.
   */
  function hostAllowed(req) {
    const host = String(req.headers.host ?? "").toLowerCase();
    const bare = host.startsWith("[") ? host.slice(0, host.indexOf("]") + 1) : host.split(":")[0];
    if (LOOPBACK_HOSTS.has(bare)) return true;
    const extras = Array.isArray(config.allowedHosts)
      ? config.allowedHosts.map((h) => String(h).toLowerCase().replace(/^https?:\/\//, "").split("/")[0].split(":")[0])
      : [];
    return extras.includes(bare);
  }

  /** Origin gate: when a browser sends an Origin header it must match Host. */
  function originAllowed(req) {
    const origin = String(req.headers.origin ?? "");
    if (origin === "") return true; // no Origin header (GET, curl, same-origin)
    try {
      ...
      () =>
        ...
        handler: async (req, res) => {
          try {
            ...
            // Reject foreign Hosts (DNS rebinding / proxy forwarding) and
            // cross-origin requests before any sensitive route answers.
            ...
            if (sub === "settings" && req.method === "GET") {
              return sendJson(res, 200, { ok: true, settings: safeSettings() });
            }
            if (sub === "settings" && req.method === "POST") {
              ...
              return sendJson(res, 200, { ...result, settings: result.ok ? safeSettings() : undefined });
            }
            if (sub === "cookie" && req.method === "POST") {
              ...
              const result = saveSettings({ cookie: value });
              return sendJson(res, 200, { ...result, settings: result.ok ? safeSettings() : undefined });
            }
            if (sub === "logout" && req.method === "POST") {
              await apiGet("/logout", {});
              const result = saveSettings({ cookie: "" });
              return sendJson(res, 200, { ...result, settings: result.ok ? safeSettings() : undefined });
            }
