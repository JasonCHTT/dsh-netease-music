...
**Q：登录状态会被别人拿到吗？**
不会。登录 Cookie 只保存在你的本机 profile 目录，且只存在于宿主机进程内部——浏览器端拿到的设置数据已剔除 Cookie 字段，插件路由也只响应本机（loopback）请求。登录状态不会随插件仓库或安装包分发。

**Q：本地 API 服务（默认 3000 端口）可以暴露到公网吗？**
不可以。登录 Cookie 以 URL query 参数形式传给本机 NeteaseCloudMusicApi（上游 API 的设计约束），若把该端口暴露到公共网络，服务器日志与网络抓包都可能残留 Cookie。请保持该服务仅监听本机（127.0.0.1），不要把 3000 端口转发到公网。
